...
class AnalysisManager(object):
    ...
    def comments_and_posts(self):
        ...
        seven_day_change, seven_day_total = com_post.get_comments_posts_activity(7)
        thirty_day_change, thirty_day_total = com_post.get_comments_posts_activity(30)
    ...
    def bigramfreq(self):
        ...
        # Retrieve documents with old word freq 
        subreddit_doc = self._db.get_documents("subreddits", self.subreddit)
        bigramfreqbd_doc = self._db.get_documents("bigramfreqbd", self.subreddit)
        ...
        if "word_count" in subreddit_doc:
            old_bigram_freq = subreddit_doc["word_count"]
    ...
    def currency_mentions(self):
        ...
        # Retrieve documents with old word freq 
        subreddit_doc = self._db.get_documents("subreddits", self.subreddit)
        currencymentionsbd_doc = self._db.get_documents("currencymentionsbd", self.subreddit)

        old_currency_mentions = None
        if "currency_mentions" in subreddit_doc:
            old_currency_mentions = subreddit_doc["currency_mentions"]
